...
@section('content')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Debug: Log all milestone checkboxes found
            ...
            checkbox.addEventListener('change', function() {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        const goalCard = document.querySelector(`#goal-${goalId}`);
                        if (goalCard) {
    ...
    document.querySelectorAll('.milestone-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            .then(data => {
                if (data.success) {
                    // Find the correct goal card and update its progress bar
                    const goalCard = document.querySelector(`#goal-${goalId}`);
                    if (!goalCard) {
                        // Try alternative selector if ID-based selector fails
                        const goalCard = document.querySelector(`.goal-card[data-goal-id="${goalId}"]`);
                    }
                    ...
                    console.log('Progress updated:', data.progress, 'Status:', data.status);
                }
            })
            .catch(error => {
                console.error('Error toggling milestone:', error);
